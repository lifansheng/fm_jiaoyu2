...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        $this->imessage('更新幻灯片成功！', $this->createWebUrl('newbanners', array('op' => 'display')), 'success');
    }
    ...
    if (empty($banner)) {
        $this->imessage('抱歉，幻灯片不存在或是已经被删除！', $this->createWebUrl('newbanners', array('op' => 'display')), 'error');
    }
    pdo_delete($this->table_banners, array('id' => $id));
    $this->imessage('幻灯片删除成功！', $this->createWebUrl('newbanners', array('op' => 'display')), 'success');
} else {
    $this->imessage('请求方式不存在');
}
include $this->template('web/newbanners');
?>