 ...
 */
        ...
        if ($operation == 'display') {
            ...
            if (!empty($fenzulist)) {
				foreach($fenzulist as $key => $row){
					$schoolinfo = pdo_fetch("SELECT title,logo FROM " . tablename($this->table_index) . " WHERE id = :id ", array(':id' => $row['schoolid']));
					$fenzulist[$key]['name'] = $schoolinfo['title'];
					$fenzulist[$key]['logo'] = tomedia($schoolinfo['logo']);
				}
