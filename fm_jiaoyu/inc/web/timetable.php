...
if($operation == 'post'){
    ...
    $allsd = pdo_fetchall("SELECT sid,sname FROM " . tablename($this->table_classify) . " WHERE weid = :weid And schoolid = :schoolid And type = :type ORDER BY ssort DESC", array(':weid' => $weid, ':schoolid' => $schoolid, ':type' => 'timeframe'));
    ...
    if(checksubmit('submit')){// 添加操作
        ...
        /********************生成每一天的课表***********************/
        $daydiff = diffBetweenTwoDays($_GPC['begintime'],$_GPC['endtime']);
        $weekname = ['sun','mon','tus','wed','thu','fri','sat'];
        for ($i=0; $i <= $daydiff; $i++) { 
            $begin = strtotime($_GPC['begintime']) + $i* 86400;
            $week = date("w",$begin); //今天周几
            for ($j=1; $j <= 12; $j++) { 
                $sdid = $_GPC["{$weekname[$week]}_{$j}_sd"];
                $kmid = $_GPC["{$weekname[$week]}_{$j}_km"];
                if(!empty($sdid) || !empty($kmid)){
                    //获取时段的开始和结时间
                    $sdinfo = pdo_fetch("SELECT sd_start,sd_end FROM ".GetTableName('classify')." WHERE schoolid = '{$schoolid}' AND sid = '{$sdid}' ");
                    $starttime = strtotime($_GPC['begintime'].' '.date('H:i:s',$sdinfo['sd_start'])) + $i* 86400;
                    $endtime = strtotime($_GPC['begintime'].' '.date('H:i:s',$sdinfo['end_start'])) + $i* 86400;
                    ...
                    $kebiaoid = pdo_fetch("SELECT id FROM ".GetTableName('glkebiao')." WHERE schoolid = '{$schoolid}' AND date = '{$date}' AND kbid = '{$id}' AND  num = '{$j}' ")['id'];
                    if(!empty($kebiaoid)){
                        pdo_update(GetTableName('glkebiao',false),$data,array('id'=>$kebiaoid));
                    }else{
                        pdo_insert(GetTableName('glkebiao',false),$data);
                    }
                }
            }
        }
        /********************生成每一天的课表***********************/
        /****************如果日期缩短就会执行当前操作**************/
        $startdate = strtotime($_GPC['begintime']);
        $enddate = strtotime($_GPC['endtime']);
        $delSql = "DELETE FROM ".GetTableName('glkebiao')." WHERE schoolid='{$schoolid}' AND kbid = '{$id}' AND (`date` < '{$startdate}' OR `date` > '{$enddate}')";
        pdo_run($delSql);
        /****************如果日期缩短就会执行当前操作**************/
