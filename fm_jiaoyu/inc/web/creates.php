...
if(checksubmit()){
	...
	if($_GPC['is_show'] == 1){
        $saoma = pdo_fetch("SELECT * FROM " . tablename('users') . " WHERE tid = :tid And schoolid = :schoolid", array(':tid' => 0,':schoolid' => $schoolid));
        if(!empty($saoma)){
            $this->imessage('抱歉,本校只能创建一个扫码账户', referer(), 'error');
        }
	}
    if(empty($group)){
    ...
    if(!$tuid){//新增
        ...
        if($restuid > 0 && is_numeric($restuid)){
			...
			pdo_update($this->table_teachers, array('password' => trim($_GPC['password'])), array('id' => $_GPC['jsid']));
            $this->imessage('用户增加成功！', referer(), 'success');
        }else{
            $this->imessage($restuid['message'], referer(), 'error');
        }
    }else{
        $user['uid'] = $tuid;
