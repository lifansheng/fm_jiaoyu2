...
$schoolset              = pdo_fetch("SELECT * FROM " . tablename($this->table_schoolset) . " WHERE schoolid = '{$schoolid}' AND weid= '{$weid}'");
...
if($operation == 'display'){
    ...
    if($_GPC['type'] == 3){
        $condition .= " AND type = '异常进校' ";
    }
    if($_GPC['type'] == 4){
        $condition .= " AND type = '异常离校' ";
    }
    if($_GPC['shenfen'] == 1){
        $condition .= " AND sid > 0 ";
    }
    ...
    foreach($list as $index => $row){
        ...
        $qdtid                   = $row['qdtid'] ? pdo_fetch("SELECT tname FROM " . tablename($this->table_teachers) . " WHERE id = '{$row['qdtid']}' "): [];
        $idcard                  = $row['cardid'] ?  pdo_fetch("SELECT pname,cardtype FROM " . tablename($this->table_idcard) . " WHERE idcard = '{$row['cardid']}' And schoolid = '{$schoolid}' ") : [];
        ...
        $list[$index]['qdtname'] = $qdtid['tname'] ? $qdtid['tname'] : '';
		...
		if (preg_match('/(http:\/\/)|(https:\/\/)/i', $row['pic']) && !keep_wt() ) {
		...
		if (preg_match('/(http:\/\/)|(https:\/\/)/i', $row['pic2']) && !keep_wt()) {
    ...
    if($_GPC['out_put'] == 'output'){
        ...
        foreach($list as $index => $row){
            $student             = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " WHERE id = '{$row['sid']}' ");
            $teacher             = pdo_fetch("SELECT tname FROM " . tablename($this->table_teachers) . " WHERE id = '{$row['tid']}' ");
            $idcard              = pdo_fetch("SELECT pname,usertype,pard FROM " . tablename($this->table_idcard) . " WHERE idcard = '{$row['cardid']}' ");
			...
			if(!empty($idcard['pname'])){
				$ppname = $idcard['pname'];
			}else {
				if(!empty($student['s_name']) ){ //学生
					$ppard = $list['pard'] == 2 ? '母亲' : ($list['pard'] == 3 ? '父亲' : ( $list['pard'] == 4 ? '本人' : '家长' )  );
					$ppname = $student['s_name'].$ppard;
				}else {
					$ppname = $teacher['tname'];

				}
			}

            $arr[$ii]['pnames']  = $ppname;
...
}elseif($operation == 'exceldd'){
    $params = array(
        'year' => $_GPC['excel_year'],
        'month' => $_GPC['excel_month'],
        'schoolid' => $schoolid,
        'weid' => $weid,
        'excelType' => intval($_GPC['excelType'])
    );
    $test = new ExtraExcel();
    $test->DDExtraExcelWriter($params);
    die();
}else{
