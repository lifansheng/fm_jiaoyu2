        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
				...
				if($_GPC['thumb']){
					...
					if(count($thumbs) >= 10){
						$this->imessage('抱歉，引导图组不能大于9张', $this->createWebUrl('newguid', array('op' => 'display')), 'error');
					}
				}else{
					if(empty($_GPC['old_thumb'])){
						$this->imessage('抱歉，图片不能为空！', $this->createWebUrl('newguid', array('op' => 'display')), 'error');
					}
				}
                if (!empty($id)) {
                ...
                $this->imessage('更新成功！', $this->createWebUrl('newguid', array('op' => 'display')), 'success');
            }
            ...
            if (empty($banner)) {
                $this->imessage('抱歉，不存在或是已经被删除！', $this->createWebUrl('newguid', array('op' => 'display')), 'error');
            }
            pdo_delete($this->table_banners, array('id' => $id));
            $this->imessage('删除成功！', $this->createWebUrl('newguid', array('op' => 'display')), 'success');
        } else {
            $this->imessage('请求方式不存在');
        }
		include $this->template('web/newguid');
?>