...
if(!empty($it)){
	$img_url = array();
	$iii = 0 ;
	// $list = pdo_fetchall("SELECT * FROM " . GetTableName('psychology') . " where weid = :weid AND schoolid = :schoolid AND (userid = :userid AND touserid = :touserid OR (userid = :touserid AND touserid = :userid)) ORDER BY createtime ASC ", array(':weid' => $weid, ':schoolid' => $schoolid, ':userid' => $thisleave['userid'], ':touserid' => $thisleave['touserid']));	
	$leaveid = pdo_fetch("SELECT leaveid FROM ".GetTableName('psychology')." WHERE id='{$id}' AND schoolid = '{$schoolid}' ")['leaveid'];	
    $list = pdo_fetchall("SELECT * FROM " . GetTableName('psychology') . " where weid = :weid AND schoolid = :schoolid AND leaveid = :leaveid ORDER BY createtime ASC ", array(':weid' => $weid, ':schoolid' => $schoolid, ':leaveid' => $leaveid));		
	foreach ($list as $k => $v) {
    ...
    $lasttime = pdo_fetch("SELECT id,createtime FROM " . GetTableName('psychology') . " where weid = :weid AND schoolid = :schoolid AND (userid = :userid AND touserid = :touserid OR (userid = :touserid AND touserid = :userid)) ORDER BY createtime DESC ", array(':weid' => $weid, ':schoolid' => $schoolid, ':userid' => $thisleave['userid'], ':touserid' => $thisleave['touserid']));	
	include $this->template(''.$school['style3'].'/psychology/tpsychology');
}else{
	session_destroy();
