 ...
 */
		...
		$thisleave = pdo_fetch("SELECT userid,touserid FROM " . tablename($this->table_liuyan) . " where weid = :weid AND id = :id ", array(':weid' => $weid, ':id' => $id));
		$student = pdo_fetch("SELECT s_name,icon,bj_id FROM " . tablename($this->table_students) . " where weid = :weid AND id = :id", array(':weid' => $weid, ':id' => $it['sid']));
		$id = $it['sid'];
		if(!empty($it)){
			$list = pdo_fetchall("SELECT * FROM " . tablename($this->table_liuyan) . " where weid = :weid AND leaveid = :leaveid ORDER BY createtime ASC ", array(':weid' => $weid, ':leaveid' => $id));	
			$img_url = array();
			$iii = 0 ;
			foreach ($list as $k => $v) {
				...
				if($v['touserid'] == $it['id']){
					...
					if($v['isread'] ==1){
						pdo_update($this->table_liuyan, array('isread' =>  2), array('id' =>  $v['id']));
					}		
				}	
				...
				if ($users['sid']){
					...
					$list[$k]['name'] = $student['s_name'];
					$list[$k]['icon'] =  empty($teacher['thumb']) ? $school['spic'] : $student['thumb'];					
				}
				if(!empty($v['audio'])){
			...
			$lasttime = pdo_fetch("SELECT id,createtime FROM " . tablename($this->table_liuyan) . " where weid = :weid AND leaveid = :leaveid ORDER BY createtime DESC ", array(':weid' => $weid, ':leaveid' => $id));
			$this->checkobjiect($schoolid, $it['sid'], $obid);
			include $this->template(''.$school['style2'].'/sliuyan');
        }else{
			session_destroy();
