 ...
 */
		...
		$schoolset = pdo_fetch("SELECT is_unbind,addedinfo FROM " . tablename($this->table_schoolset) . " where weid = :weid AND schoolid=:schoolid ", array(':weid' => $weid, ':schoolid' => $schoolid));
        if(!empty($it)){
